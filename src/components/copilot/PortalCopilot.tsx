...
function CopilotMessagePanels({
  ...
  const bundle = (messageId ? panelsByMessageId[messageId] : undefined) ?? (isLastMessage ? pendingBundle ?? undefined : undefined)
  ...
  return (
    <ComposerPrimitive.Root className="mx-auto flex w-full max-w-3xl flex-col gap-3 rounded-[1.75rem] border bg-card/95 p-3 shadow-lg">
      <ComposerPrimitive.Input
        ...
        maxRows={8}
        minRows={2}
        placeholder="Ask about follow-up, quote policy, what to do next, or launch a tour…"
      />
...
export function PortalCopilot() {
  ...
  const { close, isOpen, open } = usePortalCopilot()
            ...
            <header className="flex items-center justify-between gap-3 border-b px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Internal docs, assigned tasks, stale follow-up, and tour launchers in one panel.
                </p>
              </div>
              <button
