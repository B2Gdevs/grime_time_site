...
export async function GrimeTimeMarketingHome({
  ...
  const heroEyebrow = page.hero.eyebrow?.trim() || 'Grime Time exterior cleaning'
  const heroHeadlinePrimary = page.hero.headlinePrimary?.trim() || 'Clear scope.'
  const heroHeadlineAccent = page.hero.headlineAccent?.trim() || 'Visible results.'
  const heroPanelEyebrow = page.hero.panelEyebrow?.trim() || 'Fast lane for homeowners'
  const heroPanelHeading = page.hero.panelHeading?.trim() || 'Quotes and scheduling without vague contractor talk.'
  const heroPanelBody =
    page.hero.panelBody?.trim() ||
  ...
  return (
    <div className="w-full">
      <PageComposerCanvasSection index={-1} label="Hero" supportsInsertionAbove={false}>
