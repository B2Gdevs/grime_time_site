...
import {
  extractPreviewState,
  getPreviewBlocksStructuredContent,
} from "./preview-tools";
...
describe("handleMockToolCall", () => {
  ...
  it("lists the seeded preview blocks", async () => {
    const result = await handleMockToolCall("list_blocks", {});
  ...
  it("returns widgetState payload for fixture switching", async () => {
    const result = await handleMockToolCall("set_preview_block", {
      blockType: "service-grid",
      fixtureId: "featureCards",
    });

    assert.equal(result.isError, undefined);
    assert.deepEqual(extractPreviewState(result), {
  ...
  it("returns tool-driven overrides for custom JSON preview", async () => {
    const result = await handleMockToolCall("apply_preview_overrides", {
      fixtureId: "pricingSteps",
      block: {
        heading: "Pricing copy stress test",
      },
    });
