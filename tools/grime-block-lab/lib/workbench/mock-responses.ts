...
import {
  buildApplyPreviewOverridesResponse,
  buildSetPreviewBlockResponse,
...
const mockHandlers: Record<string, MockHandler> = {
  ...
  list_blocks: async () => {
    ...
    return {
      structuredContent: getPreviewBlocksStructuredContent(),
      content: "Listed block fixtures for the current lab.",
    };
  },

  set_preview_block: async (args) => {
    await simulateDelay(150);
    return buildSetPreviewBlockResponse(args);
  },

  apply_preview_overrides: async (args) => {
    await simulateDelay(180);
    return buildApplyPreviewOverridesResponse(args);
  },
};
